...
or increase socketAcquisitionWarningTimeout=(millis) in the NodeHttpHandler config.`
  ...
  private resolveDefaultConfig(options?: NodeHttpHandlerOptions | void): ResolvedNodeHttpHandlerConfig {
    ...
    const maxSockets = 50;
    console.log(`Smithy Debug Log: NodeHttpHandler - Resolving default config with requestTimeout=${requestTimeout}, connectionTimeout=${connectionTimeout}, socketTimeout=${socketTimeout}, socketAcquisitionWarningTimeout=${socketAcquisitionWarningTimeout}, httpAgent=${httpAgent}, httpsAgent=${httpsAgent}`);
  ...
  async handle(request: HttpRequest, { abortSignal }: HttpHandlerOptions = {}): Promise<{ response: HttpResponse }> {
    ...
    return new Promise((_resolve, _reject) => {
      ...
      const resolve = async (arg: { response: HttpResponse }) => {
        ...
        console.log(`Smithy Debug Log: NodeHttpHandler - Request resolved with response status code ${arg.response.statusCode}`);
        _resolve(arg);
      };
      const reject = async (arg: unknown) => {
        await writeRequestBodyPromise;
        timeouts.forEach(timing.clearTimeout);
        console.log(`Smithy Debug Log: NodeHttpHandler - Request rejected with error`, arg);
        _reject(arg);
      };
